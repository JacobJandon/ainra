...
ci: fmt clippy lockfile-sync soak-ingest reasons-check corpus-check instance-gate claims-check policy-parity test vectors diff conformance cli-check suite-migration-drill ceremony-rehearsal-multi witness-check push-advisory-check changelog-board-check sdk-test site site-check
...
skills-replay: sdk-build cli-deps
...
# The P0 reference CLI has dependencies of its own (@noble/post-quantum) that NOTHING installed. It ran on any
# machine where someone had once done it by hand, and failed from a cold clone — found by `make succession-drill`.
cli-deps:
	cd apps/cli-node && [ -d node_modules ] || npm install --prefer-offline --no-audit --no-fund --silent
...
instance-gate: wedge-build  ## the middleware accepts a running copy and refuses an unentitled one BY NAME
...
# defect lived somewhere else. See docs/CLAIMS.md.
claims-check:    ## every public claim agrees with its source of truth, everywhere it is asserted
...
# milestones stale, because those edits touched no tracked claim. Claim parity and deploy currency are
# different properties. This is the second one. Needs network.
deploy-current:  ## production must serve exactly what this repo builds (byte-for-byte)
	@node tools/deploy-current.mjs $(if $(BASE),--base $(BASE),)
