...
Single-key root (threshold ceremony = v0.2) · **Ed25519 signatures only** — the standard mandates hybrid
Ed25519 + ML-DSA-65, and the Rust core + browser SDK already do it; hybrid in this downloadable CLI is the
v0.2 flagship · 3 witness keys generated locally and labeled `(local)` (independent witnesses pending) ·
status list is poll-based (push fabric pending). Everything else is real: tamper with any byte of a passport,
cert, or log line and verification fails.
...
Apache-2.0. Specification: AINRA Standard v5.1.
