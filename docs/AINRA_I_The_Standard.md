...
**3. Status line (honest, live).** ROOT: PRE-CEREMONY · LOGS SEALED: 0 · WITNESSES: RECRUITING · SPEC: PUBLIC (this document) · REFERENCE CLI: v0.1.0 WORKING (real Ed25519 chain verification, signed revocation, hash-chained checkpointed log; labeled limits: single-key root pending threshold ceremony, local witness keys pending independent witnesses, poll-based status pending push fabric). Every public number is a cited external figure or a shipped artifact; an honest zero beats a decorated maybe.
