...
**ADR-018 Push status — NEW: advisory transport, sovereign pull.** *Problem:* revocation must fail closed in <60 s (ADR-017 leans on this), so a verifier wants to hear about a fresh head/delta *fast* — but a push channel that a verifier *trusts* would become a new authority to attack. *Alternatives:* (a) verifier trusts a pushed head/delta directly (fast, but a forged or withheld push becomes a revocation bypass); (b) pull-only on a fixed schedule (safe, but revocation latency is the whole poll interval); (c) **push as advisory transport over a sovereign pull**. *Decision:* **(c).** An optional SSE/webhook feed MAY announce "a new head/delta exists at seq N", but the announcement is **unsigned and carries no trust**. The verifier reacts by running the **normal signed pull + validation** — delegate signature then freshness — identically to a scheduled poll; there is **no push-only code path** and the pushed bytes are never believed. This makes push a pure latency optimization: it can only make the sovereign pull happen *sooner*, never replace it. *Two threat cases are the rationale, and both fail closed:* **suppression** — an attacker withholds the push so the verifier "never hears" of a change; it changes nothing, because the scheduled pull enforces **freshness**, so a head not refreshed past its window is `stale_status` (a revocation cannot be hidden beyond the freshness class — F1 ≤ 30 s). **forgery** — an attacker forges a push announcing a rosy head/delta; it changes nothing, because the announced bytes are validated on pull and a forged signature is `checkpoint_invalid` (a revocation cannot be injected either). The SDK's optional subscriber is a thin wrapper that, on any hint, triggers only the ordinary refresh path. Proven end to end by the ADR-018 threat harness (`make push-advisory-check`) over the real conformance vectors; D-038.
