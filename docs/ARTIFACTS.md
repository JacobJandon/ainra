...
|---|---|---|
| `vectors/v1` (1105) · `vectors/v1-delta` (17) · `vectors/v1-directory` (9) | `ainra-vector-gen` | seeded `ChaCha20Rng` (fixed seeds; no clock, no OS RNG) |
...
- **The SDK `dist/`** (`packages/sdk-ts/dist`) — a `tsc` build output whose exact bytes depend on the TypeScript
  ...
  proves `dist` agrees with `ainra-core` on all 1105 + 17 + 9 vectors. Byte-pinning a compiler output would couple the
