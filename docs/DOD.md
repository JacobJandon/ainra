...
| **≥3 external verifiers** | ⏳ external (machinery ready) | **`kits/verifier/`** lets any stranger verify root-dark + reject revoked/forged with only `@ainra/sdk`, then verify a **fresh challenge corpus** (K bundles, secret coin-flip revocations we never publish) and emit a signed **execution-bound attestation**. `check-attestation.mjs --secret <answer-key>` certifies it only if the fresh-bundle verdicts match a private answer key (a non-executor must guess all K → 2^-K); hand-authored/conformance-only/wrong-answer/answer-key-less all fail closed. Bar = 3 attestations under distinct keys, one challenge each, 3 machines. Honest scope: proves *actual verification performed on un-precomputable inputs* — NOT the exact binary, NOT Sybil (distinctness = one-challenge-per-vetted-party). (kill-gate K4; GENESIS-CHECKLIST §3) |
