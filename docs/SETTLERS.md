...
`not-logged-binding-0001` runs in the **1153-vector corpus across four implementations** on every board. Their
...
*"canonicalization attack/fragility surface"*. One rulebook, 1153 vectors under it.
...
|---|---|---|---|
| ~~**R1**~~ | ~~**Build the instance-credential rung, or stop citing it.**~~ **DONE — M28 (ADR-019 / D-047).** The rung is implemented in all four implementations, pinned by the corpus, and carried to every developer-facing surface in M29. ADR-017's 366-day passport is now safe for the reason it always claimed: a running copy carries a minutes-to-hours credential, not the lineage key. | Undetected key compromise ran for a year | Closed 2026-08 |
| ~~**R2**~~ | **DONE — D-044.** Graduated distrust keyed on the transparency-log **leaf index**, not on a date. Absent = trusted; present = refuse what the registrar logged at index ≥ n, everything earlier still verifies. Enforced in all three implementations after inclusion is proven, for the credential and every hop. 48 new vectors; four-way differential 1153/1153. The web PKI keyed this on `notBefore` and a CA backdated to evade it — a log index cannot be backdated. | A registrar too large to remove; a registry with no state between listed and gone | **Shipped** |
...
| **R6** | **Recruit witnesses, or stop describing detection as a control.** Currently honest — the site and footer state `0 OPERATORS — GAP` and `SPLIT-VIEW GUARANTEE NOT IN FORCE`, and `make claims-live` holds the deployment to that. Keep it that way until N≥3 operate. | Claiming a control with zero operators | Still open — row one of the campaign |
